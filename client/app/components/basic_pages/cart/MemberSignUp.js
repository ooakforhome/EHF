  ...
  productsBox(userId){
    ...
    JSON.parse(localStorage.cart).map(item=>{
      productsInfo.push({
        ...
        purchase_price: item.retail,
        quantity: item.quantity
      })
    })
    axios.post('/api/placeorder', {
      products: productsInfo,
      customer_name: this.state.userRegisterForm.username.value,
      customer_email: this.state.userRegisterForm.email.value,
      shipping_address: {
        recipient_name: this.state.address.recipient_name.value,
        address1: this.state.address.address1.value,
        address2: this.state.address.address2.value,
        city: this.state.address.city.value,
        state: this.state.address.state.value,
        zipcode: this.state.address.zipcode.value,
        phone: this.state.address.phone.value
      },
      user: userId
    })
    .then(orderData => {
      ...
      axios.post('/api/member/signup', {
        ...
        .then(res => {
          if(res.data.success){
            ...
            this.productsBox(res.data.userId)
          }
        })
  }
